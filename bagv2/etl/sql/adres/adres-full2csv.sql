...
\COPY (SELECT openbareruimtenaam as openbareruimte,huisnummer,huisletter,huisnummertoevoeging, postcode, woonplaatsnaam as woonplaats,gemeentenaam as gemeente,provincienaam as provincie,nummeraanduiding,verblijfsobjectgebruiksdoel,oppervlakteadresseerbaarobject,adresseerbaarobjectstatus as verblijfsobjectstatus,adresseerbaarobject as object_id,typeadresseerbaarobject as object_type,nevenadres,pandid,pandstatus,pandbouwjaar,trim_scale(ROUND(ST_X(geopunt)::numeric,3)) as x,trim_scale(ROUND(ST_Y(geopunt)::numeric,3)) as y, trim_scale(ROUND(ST_X(ST_Transform(geopunt, 4326))::numeric,8)) as lon, trim_scale(ROUND(ST_Y(ST_Transform(geopunt, 4326))::numeric,7)) as lat,verkorteopenbareruimtenaam as verkorteopenbareruimte FROM adres_full ORDER BY postcode,openbareruimtenaam,huisnummer,huisletter,huisnummertoevoeging) TO '/tmp/bagadres-full.csv' WITH CSV HEADER DELIMITER ';'
