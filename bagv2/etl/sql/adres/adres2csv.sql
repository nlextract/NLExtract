...
\COPY (SELECT openbareruimtenaam as openbareruimte,huisnummer,huisletter,huisnummertoevoeging,postcode,woonplaatsnaam as woonplaats,gemeentenaam as gemeente,provincienaam as provincie, adresseerbaarobject as object_id,typeadresseerbaarobject as object_type,nevenadres,ROUND(ST_X(geopunt)::numeric,3)::double precision as x,ROUND(ST_Y(geopunt)::numeric,3)::double precision as y, ROUND(ST_X(ST_Transform(geopunt, 4326))::numeric,8)::double precision as lon, ROUND(ST_Y(ST_Transform(geopunt, 4326))::numeric,8)::double precision as lat  FROM adres ORDER BY postcode,openbareruimtenaam,huisnummer,huisletter,huisnummertoevoeging) TO '/tmp/bagadres.csv' WITH CSV HEADER DELIMITER ';'
