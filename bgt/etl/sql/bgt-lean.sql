...
--
-- nov 2023: https://github.com/nlextract/NLExtract/issues/363
-- use ST_Buffer(0) to include and repair invalid (surface i.e. column 'geometrie_vlak') geometries
-- expand to MultiPolygon as many will be of that type as a result of ST_Buffer(0)
--
...
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_punt is not null ;
...
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_vlak is not null;
CREATE INDEX buurt_vlak_geometrie_vlak_geom_idx ON bgt_lean.buurt_vlak USING gist(geometrie_vlak);
...
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_multipunt is not null;
...
SELECT
    a.*,
    ST_CurveToLine(b.geometrie_multivlak)::geometry(MultiPolygon, 28992) as geometrie_multivlak
FROM bgt_lean.kunstwerkdeel_cols a, latest.kunstwerkdeelactueelbestaand b
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_multivlak is not null;
...
SELECT
    ...
    ST_Multi(ST_Buffer(ST_CurveToLine(b.geometrie_vlak), 0))::geometry(MultiPolygon, 28992) as geometrie_vlak
FROM bgt_lean.kunstwerkdeel_cols a, latest.kunstwerkdeelactueelbestaand b
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_vlak is not null AND ST_IsValid(b.geometrie_vlak) is not False ;
...
SELECT
    a.*,
    ST_CurveToLine(b.geometrie_lijn)::geometry(LineString, 28992) as geometrie_lijn
FROM bgt_lean.kunstwerkdeel_cols a, latest.kunstwerkdeelactueelbestaand b
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_lijn is not null;
...
DROP TABLE IF EXISTS bgt_lean.openbareruimte_vlak CASCADE;
CREATE TABLE bgt_lean.openbareruimte_vlak AS
SELECT
    a.*,
    ST_Multi(ST_Buffer(ST_CurveToLine(b.geometrie_vlak), 0))::geometry(MultiPolygon, 28992) as geometrie_vlak
FROM bgt_lean.openbareruimte_cols a, latest.openbareruimteactueelbestaand b
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_vlak is not null;
CREATE INDEX openbareruimte_vlak_geometrie_vlak_geom_idx ON bgt_lean.openbareruimte_vlak USING gist(geometrie_vlak);
-- CREATE INDEX openbareruimte_vlak_lokaalid_idx ON bgt_lean.openbareruimte_vlak USING btree (lokaalid);
ALTER TABLE ONLY bgt_lean.openbareruimte_vlak ADD CONSTRAINT openbareruimte_vlak_pkey PRIMARY KEY (ogc_fid);
...
DROP TABLE IF EXISTS bgt_lean.overigbouwwerk_vlak CASCADE;
CREATE TABLE bgt_lean.overigbouwwerk_vlak AS
SELECT
    a.*,
    ST_Multi(ST_Buffer(ST_CurveToLine(b.geometrie_vlak), 0))::geometry(MultiPolygon, 28992) as geometrie_vlak
FROM bgt_lean.overigbouwwerk_cols a, latest.overigbouwwerkactueelbestaand b
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_vlak is not null;
CREATE INDEX overigbouwwerk_vlak_geometrie_vlak_geom_idx ON bgt_lean.overigbouwwerk_vlak USING gist(geometrie_vlak);
-- CREATE INDEX overigbouwwerk_vlak_lokaalid_idx ON bgt_lean.overigbouwwerk_vlak USING btree (lokaalid);
ALTER TABLE ONLY bgt_lean.overigbouwwerk_vlak ADD CONSTRAINT overigbouwwerk_vlak_pkey PRIMARY KEY (ogc_fid);
...
DROP TABLE IF EXISTS bgt_lean.pand_vlak CASCADE;
CREATE TABLE bgt_lean.pand_vlak AS
SELECT
    a.*,
    ST_Multi(ST_Buffer(ST_CurveToLine(b.geometrie_vlak), 0))::geometry(MultiPolygon, 28992) as geometrie_vlak
FROM bgt_lean.pand_cols a, latest.pandactueelbestaand b
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_vlak is not null;
CREATE INDEX pand_vlak_geometrie_vlak_geom_idx ON bgt_lean.pand_vlak USING gist(geometrie_vlak);
-- CREATE INDEX pand_vlak_lokaalid_idx ON bgt_lean.pand_vlak USING btree (lokaalid);
ALTER TABLE ONLY bgt_lean.pand_vlak ADD CONSTRAINT pand_vlak_pkey PRIMARY KEY (ogc_fid);
...
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_nummeraanduiding is not null;
...
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_vlak is not null;
CREATE INDEX stadsdeel_vlak_geometrie_vlak_geom_idx ON bgt_lean.stadsdeel_vlak USING gist(geometrie_vlak);
...
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_vlak is not null;
CREATE INDEX waterschap_vlak_geometrie_vlak_geom_idx ON bgt_lean.waterschap_vlak USING gist(geometrie_vlak);
...
WHERE a.ogc_fid = b.ogc_fid AND b.geometrie_vlak is not null;
CREATE INDEX wijk_vlak_geometrie_vlak_geom_idx ON bgt_lean.wijk_vlak USING gist(geometrie_vlak);
