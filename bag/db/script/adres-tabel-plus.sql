--
-- Dit script is gemaakt door Peter van Wee (github: PeeWeeOSM) in 2020
--
-- Just van den Broecke - kleine aanpassingen, m.n. geen "bagactueel" schema verwijzingen (kan met PG search_path)
--
...
CREATE table adres_vergelijk AS 
 SELECT 
...
BEGIN;
delete from Adresselectie   A
where A.uniq_key in (
select distinct z.uniq_key from(
select uniq_key,  count(*) from adresselectie
group by uniq_key
having count(*) > 1) Z) 
...
/*
select count(*) from  adres_plus; "count" 9147695
select count(*) from  adresselectie;   "count" 9147695
*/
