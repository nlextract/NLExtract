...
DROP VIEW IF EXISTS bagactueel.nummeraanduidingactueelbestaand_compleet cascade;
CREATE OR REPLACE VIEW bagactueel.nummeraanduidingactueelbestaand_compleet AS
Select 
	NAD.gid,
	NAD.identificatie,
	NAD.aanduidingrecordinactief,
	NAD.aanduidingrecordcorrectie,
	NAD.officieel,
	NAD.inonderzoek,
	NAD.documentnummer,
	NAD.documentdatum,
...
from bagactueel.nummeraanduiding NAD
inner join (
Select identificatie, max(begindatumtijdvakgeldigheid) as begindatumtijdvakgeldigheid from bagactueel.nummeraanduiding NAD
...
BEGIN; 
--  In de view bagactueel.provincie_gemeenteactueebestaand ontbreken begin van het jaar de gemeentes die ophouden te bestaan op de eerste van het jaar. Als dit script gedraaid wordt begin januari gebaseerd op een postgis dump van december 
-- dan ontbreken opeens die gemeentes. Om dat te voorkomen maak ik een view bagactueel.provincie_gemeenteactueel  
SELECT PRINT_NOTICE('start: 2 provincie_gemeenteactueel: '||current_time);
DROP VIEW IF EXISTS bagactueel.provincie_gemeenteactueel cascade;
CREATE OR REPLACE VIEW bagactueel.provincie_gemeenteactueel as 
select a.* from (
 SELECT 
    pg.*,     rank() OVER (PARTITION BY gemeentecode ORDER BY  pg.einddatum desc nulls first) as Rangorde
    FROM bagactueel.provincie_gemeente pg) A
   where Rangorde =1;
/*
...
DROP TABLE  IF EXISTS bagactueel.BAG_WPL_ACTBEST_gegevens cascade;    
create table bagactueel.BAG_WPL_ACTBEST_gegevens as 
SELECT 
...
--count(*) as Aantal_VBO
FROM 
  bagactueel.nummeraanduidingactueelbestaand NAD, 
  bagactueel.openbareruimteactueelbestaand OBR, 
  bagactueel.verblijfsobjectactueelbestaand VBO,
  bagactueel.gemeente_woonplaatsactueelbestaand GEM_WPL, 
  bagactueel.provincie_gemeenteactueel  PRV, 
  bagactueel.woonplaatsactueelbestaand WPL
WHERE 
...
DROP TABLE IF EXISTS  bagactueel.Adresselectie cascade;
create table bagactueel.Adresselectie as 
SELECT 
...
left outer join   bagactueel.verblijfsobjectactueelbestaand VBO
...
left outer join  bagactueel.ligplaatsactueelbestaand LIG
-- 20200606 vervangen
on NAD.identificatie = LIG.hoofdadres ,  bagactueel.BAG_WPL_ACTBEST_gegevens WPL,
  bagactueel.openbareruimteactueelbestaand OBR
WHERE 
...
ROUND(cast(coalesce ( ST_Y(ST_Transform(vbo.geopunt, 4326)),ST_Y(ST_Transform(ST_Force3D(ST_Centroid(lig.geovlak)), 4326)), ST_Y(ST_Transform(ST_Force3D(ST_Centroid(sta.geovlak)), 4326))) as numeric),6) as lat
FROM 
bagactueel.adresseerbaarobjectnevenadresactueelbestaand NEV
inner join 
bagactueel.nummeraanduidingactueelbestaand_compleet NAD
...
left outer join  bagactueel.ligplaatsactueelbestaand LIG
on NEV.identificatie = LIG.identificatie ,bagactueel.BAG_WPL_ACTBEST_gegevens WPL,
  bagactueel.openbareruimteactueelbestaand OBR
WHERE 
...
CREATE INDEX idx_adresselectie_adrobj_id ON bagactueel.adresselectie USING btree(adresseerbaarobject_id); 
CREATE INDEX idx_adresselectie_nad_id ON bagactueel.adresselectie USING btree(nad_id); 
CREATE INDEX idx_adresselectie_uniq_key ON bagactueel.adresselectie USING btree(uniq_key); 

COMMIT;
-- 20200530 Query returned successfully with no result in 03:28 minutes.

-- Dit resultaat (Adresselectie) heeft unieke nummeraandudingen en dat willen we ook.
-- Helaas geen uniek combinaties van  openbareruimte_id, postcode, huisnummer, huisletter, huisnummertoevoeging


-- nu de adressen ophalen waarvan de uniq_key meer dan 1x voorkomt 
BEGIN; 
SELECT PRINT_NOTICE('start: 5 adres_dubbel: '||current_time);
DROP TABLE IF EXISTS  bagactueel.adres_dubbel cascade;
create table bagactueel.adres_dubbel as
Select A.* , B.aantaldubbel from bagactueel.Adresselectie A
inner join 
(
Select uniq_key, count(*) as aantaldubbel from bagactueel.Adresselectie 
group by uniq_key
having count(*) > 1
) b
...
-- Als in  bagactueel.adres_dubbel  een record uniek is  behalve op de kolom ADRESSEERBAAROBJECT_inonderzoek  dan gaan we het record waar ADRESSEERBAAROBJECT_inonderzoek=f verwijderen.
BEGIN;
delete from bagactueel.adres_dubbel D
where D.adresseerbaarobject_id in (

	  select distinct adresseerbaarobject_id
	  FROM bagactueel.adres_dubbel group by adresseerbaarobject_id
...
DROP TABLE IF EXISTS  bagactueel.adres_pand1 cascade;
create table bagactueel.adres_pand1 as 
SELECT 
...
FROM 
  bagactueel.verblijfsobjectpandactueelbestaand VBO_PND
  inner join (select distinct adresseerbaarobject_id from bagactueel.adres_dubbel where typeadresseerbaarobjectkort = 'VBO') A on VBO_PND.identificatie  = A.adresseerbaarobject_id
  inner join bagactueel.verblijfsobjectactueelbestaand VBO on VBO_PND.identificatie = VBO.identificatie
  left outer join bagactueel.pandactueel PND on VBO_PND.gerelateerdpand = PND.identificatie
...
DROP TABLE IF EXISTS  bagactueel.adres_pand2 cascade;
create table bagactueel.adres_pand2 as 
Select pnd.geovlak, a.VBO_ID , a.max_PND_ID from bagactueel.adres_pand1 A
inner join bagactueel.pandactueel PND on a.max_PND_ID = pnd.identificatie;
...
SELECT PRINT_NOTICE('start: 8 adres_te_ontdubbelen: '||current_time);
 DROP TABLE IF EXISTS  bagactueel.adres_te_ontdubbelen cascade;
create table bagactueel.adres_te_ontdubbelen as 
SELECT distinct
0 as Verwijderen,
0 as intersects,
  a.uniq_key, 
 ...
 b.max_PND_ID as PND_ID,
  ...
  a.postcode
FROM 
  bagactueel.adres_dubbel a
left outer join bagactueel.adres_pand1  b
on A.adresseerbaarobject_id = B.VBO_ID
left outer join bagactueel.adres_pand2  C
on A.adresseerbaarobject_id = C.VBO_ID;
--20200530 Query returned successfully: 22112 rows affected, 413 msec execution time.

CREATE INDEX idx_adres_te_ontdubbelen_id ON bagactueel.adres_te_ontdubbelen USING btree(NAD_id); 
COMMIT; 
...
update bagactueel.adres_te_ontdubbelen 
set intersects = 1 where NAD_ID in 
(
SELECT  distinct
  a.nad_id
FROM 
  bagactueel.adres_te_ontdubbelen a, 
  bagactueel.adres_te_ontdubbelen b
WHERE 
...
BEGIN;
-- Overzicht maken van de adressen die nog geen status verwijderen = 1 hebben en die overlappen met een ander adres
SELECT PRINT_NOTICE('start: 9 adres_vergelijk: '||current_time);
 DROP TABLE IF EXISTS  bagactueel.adres_vergelijk cascade;
CREATE table bagactueel.adres_vergelijk AS 
 SELECT 
...
case when a.nad_id > b.nad_id then 'a' else 'b' end NAD_ID_hoogst,  
  ...
  FROM bagactueel.adres_te_ontdubbelen a,
  bagactueel.adres_te_ontdubbelen b
...
where nad_id
in(
select distinct a_nad_id from bagactueel.adres_vergelijk
where
  (
...
where
  (
   (a_Pand_bestaand =  b_Pand_bestaand  and  b_typeadresseerbaarobjectkort='VBO' and  pand_ID_Hoogst = 'zelfde_pnd' and adrobj_id_hoogst = 'b')  
  )
) ;
...
where
  (
   (a_Pand_bestaand =  b_Pand_bestaand  and pand_ID_Hoogst = 'b' )
  )
) ;
...
where
  (
   (a_adresseerbaarobject_status=b_adresseerbaarobject_status and a_pandstatus=b_pandstatus and nad_id_hoogst = 'b')   
  )
) ;
...
where
  (
   (a_typeadresseerbaarobjectkort in ('LIG','STA') and b_typeadresseerbaarobjectkort='VBO' and b_pandstatus in ('Pand in gebruik (niet ingemeten)','Pand in gebruik', 'Sloopvergunning verleend'))
  )
) ;
...
where
  (
  ( a_typeadresseerbaarobjectkort='VBO' and b_typeadresseerbaarobjectkort in ('LIG','STA') and a_pandstatus in ('Pand buiten gebruik','Bouwvergunning verleend' , 'Bouw gestart'))
  )
) ;
...
Select count (distinct uniq_key) from bagactueel.adres_te_ontdubbelen ;--  9456
Select count (distinct uniq_key) from bagactueel.adres_te_ontdubbelen where verwijderen = 0;-- 9456
Select count (distinct nad_id) from bagactueel.adres_te_ontdubbelen where verwijderen = 0;  -- 9456
Select count (distinct nad_id) from bagactueel.adres_te_ontdubbelen where verwijderen = 1;  -- 9520
*/

-- verwijdere de records uit bagactueel.Adresselectie  die inmiddels in bagactueel.adres_te_ontdubbelen het kenmerk verwijderen = 1 hebben.
SELECT PRINT_NOTICE('start: 17 Delete_1 '||current_time);
delete from bagactueel.Adresselectie 
--select * from bagactueel.Adresselectie 
where nad_id in (
Select  nad_id from bagactueel.adres_te_ontdubbelen where verwijderen = 1
);
...
update bagactueel.Adresselectie  
set pchnhlht_UNIEK = 0 where pchnhlht in (
select pchnhlht  from bagactueel.Adresselectie  where postcode is not null group by pchnhlht having count(*) > 1);
...
select postcode, huisnummer from bagactueel.Adresselectie  where postcode is not null group by postcode, huisnummer having count(*) > 1);
...
SELECT PRINT_NOTICE('start: 20 ADRESSEERBAAROBJECT_aantal_NAD '||current_time);
 DROP TABLE IF EXISTS  bagactueel.ADRESSEERBAAROBJECT_aantal_NAD cascade;
create table bagactueel.ADRESSEERBAAROBJECT_aantal_NAD  as
select adresseerbaarobject_id, count(nad_id) as aantal from  bagactueel.Adresselectie  group by adresseerbaarobject_id;
--20200530 Query returned successfully: 9321129 rows affected, 01:39 minutes execution time.
COMMIT;

BEGIN;
CREATE INDEX idx_ADRESSEERBAAROBJECT_aantal_NAD_id ON bagactueel.ADRESSEERBAAROBJECT_aantal_NAD USING btree(adresseerbaarobject_id); 
--20200530 Query returned successfully with no result in 14.2 secs.
COMMIT;


-- als per uniq_ke een variant met inonderzoek t en ook f voorkomt dan die met f verwijderen
BEGIN;
delete from bagactueel.Adresselectie   A
where A.uniq_key in (
select distinct z.uniq_key from(
select uniq_key,  count(*) from bagactueel.adresselectie
group by uniq_key
having count(*) > 1) Z) 
	  
and  A.adresseerbaarobject_inonderzoek= 'f';
--20200531 Query returned successfully: 2 rows affected, 01:26 minutes execution time.
COMMIT;


-- maak primary key aan. Indien foutmelding dan hiervoor issue oplossen.
ALTER TABLE bagactueel.Adresselectie
...
DROP TABLE IF EXISTS  bagactueel.verblijfsobjectgebruiksdoelactueelbestaand_PIVOT;
create table bagactueel.verblijfsobjectgebruiksdoelactueelbestaand_PIVOT as 
select VBO_ID, 
...
FROM 
  bagactueel.verblijfsobjectgebruiksdoelactueelbestaand VBOGBD
  group by   VBOGBD.identificatie ,   VBOGBD.gebruiksdoelverblijfsobject
 ',

  'select distinct gebruiksdoelverblijfsobject from   bagactueel.verblijfsobjectgebruiksdoelactueelbestaand order by 1'
 )
 AS (
...
ALTER TABLE bagactueel.verblijfsobjectgebruiksdoelactueelbestaand_PIVOT 
...
bagactueel.pandactueelbestaand_plus_woningtype
bagactueel.VBO_woningtype_ontdubbeld 


Hier een aantal statements om gegevens vast te stellen van NAD, VBO en PND die betrekking hebben op de woonfunctie.  


Hiervoor geldt dat we alleen kijken naar adressen uit bagactueel.Adresselectie en actueel bestaand van alle relevante tabellen dus verblijfsobjectpandactueelbestaand en pandactueelbestaand
...
SELECT PRINT_NOTICE('start: 23 pandactueelbestaand_plus '||current_time);
 DROP TABLE IF EXISTS  bagactueel.pandactueelbestaand_plus cascade;
CREATE table bagactueel.pandactueelbestaand_plus AS 
 SELECT pand.gid,
	pand.identificatie,
  ...
  pand.pandstatus <> 'Niet gerealiseerd pand'::bagactueel.pandstatus AND pand.pandstatus <> 'Pand gesloopt'::bagactueel.pandstatus ;
...
BEGIN;
  CREATE INDEX idx_pandactueelbestaand_plus_id ON bagactueel.pandactueelbestaand_plus  USING btree(identificatie); 
  --20200531 Query returned successfully with no result in 42.5 secs.;
COMMIT; 


BEGIN;
CREATE INDEX idx_GEOM_pandactueelbestaand_plus_idl on bagactueel.pandactueelbestaand_plus  USING gist (geovlak);
...
BEGIN;
delete from bagactueel.pandactueelbestaand_plus D
where D.identificatie in (

select distinct identificatie from bagactueel.pandactueelbestaand_plus  group by   identificatie
 having count(*) > 1  )
...
--select identificatie, count(*) from  bagactueel.pandactueelbestaand_plus group by identificatie having count(*) >1;
om die reden gaan we zorgen dat alleen het recod overblijft met de hoogste begindatumtijdvakgeldigheid
--select * from bagactueel.pandactueelbestaand_plus  where identificatie = '0513100011124025';   -- komt helaas 2x voor dus
*/


BEGIN;
SELECT PRINT_NOTICE('start: 24 pandontdubbel'||current_time);
 DROP TABLE IF EXISTS  bagactueel.pandontdubbel cascade;
CREATE table bagactueel.pandontdubbel AS 
Select 1 as te_verwijderen, pnd.* from bagactueel.pandactueelbestaand_plus pnd
inner join 
(
select identificatie, count(*) from  bagactueel.pandactueelbestaand_plus group by identificatie having count(*) >1
...
-- select * from bagactueel.pandontdubbel;
BEGIN;
SELECT PRINT_NOTICE('start: 24b pandontdubbel update'||current_time);
update bagactueel.pandontdubbel  set te_verwijderen = 0 
where (identificatie, begindatumtijdvakgeldigheid) in (
select identificatie, max(begindatumtijdvakgeldigheid) as begindatumtijdvakgeldigheid from bagactueel.pandontdubbel group by identificatie);
...
BEGIN;
delete from bagactueel.pandactueelbestaand_plus D
where D.gid in (select gid from bagactueel.pandontdubbel where te_verwijderen =1 );
...
DROP TABLE IF EXISTS  bagactueel.woning_NAD_VBO_PND cascade;
create table bagactueel.woning_NAD_VBO_PND as 
SELECT 
...
FROM 
  bagactueel.verblijfsobjectgebruiksdoelactueelbestaand VBOGBD, 
  bagactueel.Adresselectie ADR, 
  bagactueel.verblijfsobjectpandactueelbestaand VBOPND,
  bagactueel.pandactueelbestaand_plus PND 
WHERE 
...
BEGIN;
  CREATE INDEX idx_woning_NAD_VBO_PND_NAD ON bagactueel.woning_NAD_VBO_PND  USING btree(NAD_ID); 
...
DROP TABLE IF EXISTS  bagactueel.woning_NAD_gegevens cascade;
create table bagactueel.woning_NAD_gegevens as 
...
from bagactueel.woning_NAD_VBO_PND group by nad_id;
...
DROP TABLE IF EXISTS  bagactueel.woning_VBO_gegevens cascade;
create table bagactueel.woning_VBO_gegevens as 
...
from bagactueel.woning_NAD_VBO_PND group by VBO_id;
...
DROP TABLE IF EXISTS  bagactueel.woning_PND_Extra_gegevens cascade;
create table bagactueel.woning_PND_Extra_gegevens as 
Select 
...
from bagactueel.woning_NAD_VBO_PND group by PND_id;
--20200603  Query returned successfully: 5556791 rows affected, 01:04 minutes execution time.
COMMIT;


BEGIN;
ALTER TABLE bagactueel.woning_PND_Extra_gegevens
...
DROP TABLE IF EXISTS  bagactueel.woning_NAD_PND_gegevens1 cascade;
create table bagactueel.woning_NAD_PND_gegevens1 as 
Select  nad_id, PND_ID , max(nad_in_pand) as nad_in_pand, max(PND_documentdatum) as PND_documentdatum
from bagactueel.woning_NAD_VBO_PND 
...
DROP TABLE IF EXISTS  bagactueel.woning_VBO_PND_gegevens cascade;
create table bagactueel.woning_VBO_PND_gegevens as 
...
--- 20200529  deze zou weg kunnen en daar waar PND_WOON gebrukt wordt kan ik bagactueel.pandactueelbestaand_plus gebruiken
...
BEGIN;
 ...
 DROP TABLE IF EXISTS  bagactueel.PND_WOON_RELATIES cascade;
create table bagactueel.PND_WOON_RELATIES as 
SELECT 
  PND1.identificatie as PND_ID1, 
  PND2.identificatie as PND_ID2
FROM 
  bagactueel.pandactueelbestaand_plus PND1, bagactueel.woning_PND_Extra_gegevens W1,bagactueel.woning_PND_Extra_gegevens W2,
  bagactueel.pandactueelbestaand_plus PND2
WHERE 
...
SELECT PRINT_NOTICE('start: 32 PND_WOON_RELATIES_aantal: '||current_time);
 DROP TABLE IF EXISTS  bagactueel.PND_WOON_RELATIES_aantal cascade;
create table bagactueel.PND_WOON_RELATIES_aantal as 
Select PND_ID1 , count(*) as aantal_gerelateerde_PND from bagactueel.PND_WOON_RELATIES
group by PND_ID1;
...
SELECT PRINT_NOTICE('start: 33 PND_WOON_type: '||current_time);
 DROP TABLE IF EXISTS  bagactueel.PND_WOON_type cascade;
create table bagactueel.PND_WOON_type as 
...
FROM 
  bagactueel.pnd_woon_relaties PWR, -- dit zijn alle woonpand-woonpand combinaties die spacial zijn verbonden. Dus minimaal 1 record per pand wegens verbonden met zichzelf
  bagactueel.pnd_woon_relaties_aantal AANT2, --- dit zijn het aantal panden waar een pand spacial mee verbonden is 
  bagactueel.pnd_woon_relaties_aantal AANT1, --- dit zijn het aantal panden waar een pand spacial mee verbonden is
  bagactueel.woning_PND_Extra_gegevens AANTNAD  --- dit zijn het aantal NAD waar een pand mee verbonden is
WHERE 
...
BEGIN;
ALTER TABLE bagactueel.PND_WOON_type
...
SELECT PRINT_NOTICE('start: 33b pandactueelbestaand_plus_woningtype '||current_time);
 DROP TABLE IF EXISTS  bagactueel.pandactueelbestaand_plus_woningtype cascade;
 create table bagactueel.pandactueelbestaand_plus_woningtype  as
 SELECT P.* ,
w.woningtype,
...
from bagactueel.pandactueelbestaand_plus p
left join bagactueel.PND_WOON_type W on p.identificatie  =w.pnd_id
left join bagactueel.woning_PND_Extra_gegevens E on E.pnd_id=p.identificatie;
COMMIT; 


BEGIN;
  CREATE INDEX idx_pandactueelbestaand_plus_woningtypeid ON bagactueel.pandactueelbestaand_plus_woningtype  USING btree(identificatie); 
  --20200531 Query returned successfully with no result in 42.5 secs.;
COMMIT; 
...
DROP TABLE IF EXISTS  bagactueel.VBO_woningtype  cascade;
create table bagactueel.VBO_woningtype as 
Select VBO.VBO_ID, 
...
From bagactueel.woning_VBO_PND_gegevens VBO , bagactueel.pandactueelbestaand_plus_woningtype PND_woningtype
...
-- maak PK aan. Indien foutmelding dan hiervoor corrigeren
ALTER TABLE bagactueel.VBO_woningtype 
...
--Select count(*) from bagactueel.adres_plus where woningtype = 'Verschillend';-- 3422
--select count(*) from bagactueel.VBO_woningtype  where Woningtype = '9 Verschillend'  -- 3422  en dat is logisch
...
DROP TABLE IF EXISTS  bagactueel.VBO_woningtype_ontdubbeld  cascade;
create table bagactueel.VBO_woningtype_ontdubbeld  as
...
min(w.woningtype) as min_woningtype, max(w.woningtype) as max_woningtype , count(*) as aantal from bagactueel.VBO_woningtype vbo inner join bagactueel.woning_NAD_VBO_PND X
inner join bagactueel.pandactueelbestaand_plus_woningtype W on x.pnd_id=w.identificatie
...
-- maak PK aan. Indien foutmelding dan hiervoor corrigeren
ALTER TABLE bagactueel.VBO_woningtype_ontdubbeld 
...
update bagactueel.VBO_woningtype
set woningtype = B.Ontdubbeld_woningtype from bagactueel.VBO_woningtype_ontdubbeld B
where bagactueel.VBO_woningtype.vbo_id=B.vbo_id
;
...
-- select bagactueel.VBO_woningtype.*, length( VBO_Woningtype) as lengte , right(VBO_Woningtype,length( VBO_Woningtype)-2)  as type  from bagactueel.VBO_woningtype limit 100
 -- select  VBO_Woningtype, count(*) as aantal from bagactueel.VBO_woningtype  group by VBO_Woningtype order by count(*) desc;
...
----------------------------------------------------------------------------------------------------------------
Resultaat is de tabel "bagactueel.VBO_PANDGEGEVENS"
...
-------------------------------------------------------------------------------------------------------------
--- Bepaal per VBO ID (uit bagactueel.Adresselectie) het aantal verbonden panden dus ongeacht of de VBO een woonfunctie heeft
...
DROP TABLE   if exists bagactueel.VBO_Actueelbestaand_met_aantal_verbonden_PND cascade;
Create table bagactueel.VBO_Actueelbestaand_met_aantal_verbonden_PND as 
SELECT 
...
FROM 
  bagactueel.verblijfsobjectpandactueelbestaand VBO_PND
  inner join bagactueel.verblijfsobjectactueelbestaand VBO on VBO_PND.identificatie = VBO.identificatie
   inner join (select distinct adresseerbaarobject_id from bagactueel.Adresselectie) ADR on VBO.identificatie = ADR.adresseerbaarobject_id
  left outer join bagactueel.pandactueelbestaand_plus_woningtype PND on VBO_PND.gerelateerdpand = PND.identificatie
  group by  VBO_PND.identificatie, vbo.hoofdadres, VBO.verblijfsobjectstatus;  
--20200531: Query returned successfully: 9282292 rows affected, 04:59 minutes execution time.

CREATE INDEX VBO_Actueelbestaand_met_aantal_verbonden_PND_ID ON  bagactueel.VBO_Actueelbestaand_met_aantal_verbonden_PND USING btree (VBO_ID);
-- 20200531 Query returned successfully with no result in 13.5 secs.
COMMIT; 


--- Bepaal per PND ID het aantal verbonden VBO's (uit bagactueel.Adresselectie) ongeacht of die VBO een woonfunctie heeft
BEGIN;
SELECT PRINT_NOTICE('start: 37b PND_Actueelbestaand_met_aantal_verbonden_VBO: '||current_time);
 DROP TABLE  if exists  bagactueel.PND_actueelbestaand_met_aantal_VBO cascade;
create table bagactueel.PND_actueelbestaand_met_aantal_VBO as
SELECT 
...
FROM 
  bagactueel.verblijfsobjectpandactueelbestaand VBO_PND
 inner join bagactueel.Adresselectie ADR on VBO_PND.identificatie = ADR.adresseerbaarobject_id
 inner join bagactueel.pandactueelbestaand_plus_woningtype PND on VBO_PND.gerelateerdpand = PND.identificatie
 group by  VBO_PND.gerelateerdpand;
--20200531: Query returned successfully: 6323693 rows affected, 02:11 minutes execution time.

-- maak PK aan. Indien foutmelding dan hiervoor corrigeren
ALTER TABLE bagactueel.PND_actueelbestaand_met_aantal_VBO 
...
drop  table if exists  bagactueel.VBO_PANDGEGEVENS;
create table bagactueel.VBO_PANDGEGEVENS as
SELECT 
	VBO.vbo_id, 
...
FROM 
	bagactueel.vbo_actueelbestaand_met_aantal_verbonden_pnd VBO
	left outer join   bagactueel.pnd_actueelbestaand_met_aantal_vbo PND on    VBO.max_pnd_id = PND.pnd_id
	left outer join   bagactueel.pandactueelbestaand_plus_woningtype PND_ACT on PND.pnd_id = PND_ACT.identificatie
	left outer join   bagactueel.vbo_woningtype VBO_WOON  on VBO_WOON.VBO_id =  VBO.vbo_id;
...
CREATE INDEX VBO_Pandgegevens_VBO_ID ON  bagactueel.VBO_PANDGEGEVENS USING btree (vbo_id);
...
----------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------
-- START BLOK5 AANMAKEN TABEL bagactueel.adres_plus inclusief een paar aanpassingen
----------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------
...
DROP TABLE IF EXISTS   bagactueel.adres_plus cascade;
create table bagactueel.adres_plus as
SELECT 
  ...
  rank() OVER (PARTITION BY adres.postcode,adres.huisnummer ORDER BY  VBO_GBD.woonfunctie desc nulls last  , adres.huisletter nulls first, adres.huisnummertoevoeging nulls first,   adres.NAD_ID desc ) as Rangorde_PCHN
 FROM 
  bagactueel.adresselectie ADRES
  --  voeg toe alle gebruiksdoelen aan het adres indien het een VBO betreft
  left outer join   bagactueel.verblijfsobjectgebruiksdoelactueelbestaand_pivot VBO_GBD on ADRES.adresseerbaarobject_id = VBO_GBD.vbo_id 
  --  voeg toe alle pandgegevens aan het adres indien het een VBO betreft dat aan slechts 1 pand gekoppeld is
  left outer join   bagactueel.vbo_pandgegevens VBO_PND on ADRES.adresseerbaarobject_id = VBO_PND.vbo_id
  left outer join   bagactueel.vbo_woningtype  on bagactueel.vbo_woningtype.VBO_ID = ADRES.adresseerbaarobject_id
 left outer join bagactueel.ADRESSEERBAAROBJECT_aantal_NAD ADR_NAD on adres.adresseerbaarobject_id = ADR_NAD.adresseerbaarobject_id
...
update bagactueel.adres_plus adres  set opp_adresseerbaarobject_m2 = 0 
...
CREATE INDEX adres_plus_geom_idx ON bagactueel.adres_plus USING gist (geopunt);
SELECT PRINT_NOTICE('start: 42 adres_plus_postcode: '||current_time);
CREATE INDEX adres_plus_postcode ON  bagactueel.adres_plus USING btree (postcode);
SELECT PRINT_NOTICE('start: 43 adres_plus_adreseerbaarobject_id: '||current_time);
CREATE INDEX adres_plus_adreseerbaarobject_id ON bagactueel.adres_plus USING btree (adresseerbaarobject_id);
SELECT PRINT_NOTICE('start: 44 adres_plus_nummeraanduiding_ID: '||current_time);
CREATE INDEX adres_plus_nummeraanduiding_ID ON bagactueel.adres_plus USING btree (nummeraanduiding_ID);
SELECT PRINT_NOTICE('start: 45 adres_plus_pand_id: '||current_time);
CREATE INDEX adres_plus_pand_id ON bagactueel.adres_plus USING btree (pand_id);

--CREATE INDEX adres_plus_idx ON adres_plus USING gin (textsearchable_adres);
ALTER TABLE bagactueel.adres_plus ADD PRIMARY KEY (uniq_key);

COMMIT;

----------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------
-- EINDE BLOK5 AANMAKEN TABEL bagactueel.adres_plus 
----------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------

/*
select count(*) from  bagactueel.adres_plus; "count" 9147695
select count(*) from  bagactueel.adresselectie;   "count" 9147695
*/
  --En nu nog (wegens ontbreken betrouwbare huisnummertoevoegingen in vele administraties)  per PC_HN 1 record vaststellen
  BEGIN;
  drop view  if exists  bagactueel.adres_compleet_PCHN_Uniek cascade;
  CREATE OR REPLACE VIEW bagactueel.adres_compleet_PCHN_Uniek  AS
  SELECT 
...
DROP TABLE IF EXISTS    bagactueel.pnd_woon_relaties_aantal  cascade ;
DROP TABLE IF EXISTS    bagactueel.vbo_woningtype  cascade ;
DROP TABLE IF EXISTS    bagactueel.verblijfsobjectgebruiksdoelactueelbestaand_pivot  cascade ;
DROP TABLE IF EXISTS    bagactueel.vbo_actueelbestaand_met_aantal_verbonden_pnd  cascade ;
DROP TABLE IF EXISTS    bagactueel.pnd_actueelbestaand_met_aantal_vbo  cascade ;
DROP TABLE IF EXISTS    bagactueel.vbo_pandgegevens  cascade ;
DROP TABLE IF EXISTS    bagactueel.pandactueelbestaand_plus  cascade;
DROP TABLE IF EXISTS    bagactueel.VBO_woningtype_ontdubbeld cascade;
DROP TABLE IF EXISTS    bagactueel.woning_PND_Extra_gegevens cascade;
DROP TABLE IF EXISTS    bagactueel.pnd_woon_type cascade;
