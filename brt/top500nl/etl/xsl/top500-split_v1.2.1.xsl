...
<!--
Voorbewerking TOP500NL GML Objecten

Auteur: Just van den Broecke en Frank Steggink

Dit XSLT script doet een voorbewerking op de ruwe TOP500NL GML zoals door Het Kadaster
geleverd. Dit is nodig, omdat GDAL ogr2ogr niet alle mogelijkheden van GML goed aankan.

Voornamelijk gaat het om meerdere geometrie-attributen per TOP500NL Object. Het interne
...
Dit script splitst objecten uit TOP500NL uit in geometrie-specifieke objecten.
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                xmlns:top500nl="http://register.geostandaarden.nl/gmlapplicatieschema/top500nl/1.2.1"
    ...
    <xsl:template match="/">
        <top500nl:FeatureCollectionT500NL
                xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                xmlns:top500nl="http://register.geostandaarden.nl/gmlapplicatieschema/top500nl/1.2.1"
                xmlns:brt="http://register.geostandaarden.nl/gmlapplicatieschema/brt-algemeen/1.2.0"
                xmlns:gml="http://www.opengis.net/gml/3.2"
                xsi:schemaLocation="http://register.geostandaarden.nl/gmlapplicatieschema/top500nl/1.2.1 top500nl.xsd"
                gml:id="Top500NL_FC">
            <xsl:apply-templates/>
        </top500nl:FeatureCollectionT500NL>
    </xsl:template>
    ...
    <!--
    TOP500NL Feature Types en hun geometrie-attributen
    ...
    <xsl:template match="top500nl:FeatureMember">
        <xsl:for-each select="top500nl:Wegdeel">
            <xsl:call-template name="SplitsWegdeel"/>
        </xsl:for-each>

        <xsl:for-each select="top500nl:Waterdeel">
            <xsl:call-template name="SplitsWaterdeel"/>
        </xsl:for-each>

        <xsl:for-each select="top500nl:Spoorbaandeel">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">Spoorbaandeel_Lijn</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie"/>
            </xsl:call-template>
        </xsl:for-each>

        <xsl:for-each select="top500nl:Gebouw">
            <xsl:call-template name="CopyWithSingleGeometry">
        ...
        <xsl:for-each select="top500nl:Terrein">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">Terrein_Vlak</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrieVlak"/>
            </xsl:call-template>
        </xsl:for-each>

        <xsl:for-each select="top500nl:Inrichtingselement">
            <xsl:call-template name="SplitsInrichtingselement"/>
        </xsl:for-each>

        <xsl:for-each select="top500nl:FunctioneelGebied">
            <xsl:call-template name="SplitsFunctioneelGebied"/>
        </xsl:for-each>

        <xsl:for-each select="top500nl:GeografischGebied">
            <xsl:call-template name="SplitsGeografischGebied"/>
        </xsl:for-each>

        <xsl:for-each select="top500nl:Plaats">
            <xsl:call-template name="SplitsPlaats"/>
        </xsl:for-each>

        <xsl:for-each select="top500nl:RegistratiefGebied">
            <xsl:call-template name="SplitsRegistratiefGebied"/>
        </xsl:for-each>

        <xsl:for-each select="top500nl:Hoogte">
            <xsl:call-template name="SplitsHoogte"/>
        </xsl:for-each>

        <xsl:for-each select="top500nl:Relief">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">Relief_Lijn</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:lijnGeometrie"/>
            </xsl:call-template>
        </xsl:for-each>

        <xsl:for-each select="top500nl:PlanTopografie">
    ...
    <xsl:template name="SplitsFunctioneelGebied">
        <xsl:if test="top500nl:geometrie/brt:BRTVlakMultivlakOfPunt/brt:vlakGeometrie != ''">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">FunctioneelGebied_Vlak</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie/brt:BRTVlakMultivlakOfPunt/brt:vlakGeometrie"/>
            </xsl:call-template>
        </xsl:if>
        
        <xsl:if test="top500nl:geometrie/brt:BRTVlakMultivlakOfPunt/brt:multivlakGeometrie != ''">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">FunctioneelGebied_Vlak</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie/brt:BRTVlakMultivlakOfPunt/brt:multivlakGeometrie"/>
            </xsl:call-template>
        </xsl:if>

        <xsl:if test="top500nl:geometrie/brt:BRTVlakMultivlakOfPunt/brt:puntGeometrie != ''">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">FunctioneelGebied_Punt</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie/brt:BRTVlakMultivlakOfPunt/brt:puntGeometrie"/>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="SplitsHoogte">
        <xsl:if test="top500nl:geometrie/brt:BRTLijnOfPunt/brt:lijnGeometrie != ''">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">Hoogte_Lijn</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie/brt:BRTLijnOfPunt/brt:lijnGeometrie"/>
            </xsl:call-template>
        </xsl:if>

        <xsl:if test="top500nl:geometrie/brt:BRTLijnOfPunt/brt:puntGeometrie != ''">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">Hoogte_Punt</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie/brt:BRTLijnOfPunt/brt:puntGeometrie"/>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="SplitsPlanTopografie">
        <xsl:if test="top500nl:geometrie/brt:BRTVlakLijnOfPunt/brt:vlakGeometrie != ''">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">PlanTopografie_Vlak</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie/brt:BRTVlakLijnOfPunt/brt:vlakGeometrie"/>
            </xsl:call-template>
        </xsl:if>

        <xsl:if test="top500nl:geometrie/brt:BRTVlakLijnOfPunt/brt:lijnGeometrie != ''">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">PlanTopografie_Lijn</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie/brt:BRTVlakLijnOfPunt/brt:lijnGeometrie"/>
            </xsl:call-template>
        </xsl:if>

        <xsl:if test="top500nl:geometrie/brt:BRTVlakLijnOfPunt/brt:puntGeometrie != ''">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">PlanTopografie_Punt</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie/brt:BRTVlakLijnOfPunt/brt:puntGeometrie"/>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="SplitsRegistratiefGebied">
        <xsl:if test="top500nl:geometrie/brt:BRTVlakOfMultivlak/brt:vlakGeometrie != ''">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">RegistratiefGebied_Vlak</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie/brt:BRTVlakOfMultivlak/brt:vlakGeometrie"/>
            </xsl:call-template>
        </xsl:if>
        
        <xsl:if test="top500nl:geometrie/brt:BRTVlakOfMultivlak/brt:multivlakGeometrie != ''">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">RegistratiefGebied_Vlak</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie/brt:BRTVlakOfMultivlak/brt:multivlakGeometrie"/>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="SplitsWaterdeel">
        <xsl:if test="top500nl:geometrie/brt:BRTVlakOfLijn/brt:vlakGeometrie != ''">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">Waterdeel_Vlak</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie/brt:BRTVlakOfLijn/brt:vlakGeometrie"/>
            </xsl:call-template>
        </xsl:if>

        <xsl:if test="top500nl:geometrie/brt:BRTVlakOfLijn/brt:lijnGeometrie != ''">
            <xsl:call-template name="CopyWithSingleGeometry">
                <xsl:with-param name="objectType">Waterdeel_Lijn</xsl:with-param>
                <xsl:with-param name="geometrie" select="top500nl:geometrie/brt:BRTVlakOfLijn/brt:lijnGeometrie"/>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="CopyNonGeoProps" priority="1">
        <!-- Copieer alle top500nl:* attributen, behalve de geometrieen. -->
        <xsl:copy-of
                select="top500nl:*[not(self::top500nl:geometrie)][not(self::top500nl:lijnGeometrie)][not(self::top500nl:geometrieVlak)]"/>
    </xsl:template>
