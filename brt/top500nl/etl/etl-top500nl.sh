#!/bin/bash
#
# ETL voor TOP500NL GML met gebruik Stetl.
#
# Dit is een front-end/wrapper shell-script om uiteindelijk Stetl met een configuratie
# (etl-top500nl-v1.2.1.cfg) en parameters (options/myoptions.args) aan te roepen.
#
...
# Evt via commandline overrulen: etl-top500nl.sh <my options file>
[ -f "$1" ] && options_file=$1

# Uiteindelijke commando. Kan ook gewoon "stetl -c conf/etl-top500nl-v1.2.1.cfg -a ..." worden indien Stetl installed
# python $STETL_HOME/stetl/main.py -c conf/etl-top500nl-v1.2.1.cfg -a "$pg_options temp_dir=temp max_features=$max_features gml_files=$gml_files $multi $spatial_extent"
python $STETL_HOME/stetl/main.py -c conf/etl-top500nl-v1.2.1.cfg -a $options_file
